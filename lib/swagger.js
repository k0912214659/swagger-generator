...
function parseSchema (obj) {
  ...
  if (obj.applications) {
    if (obj.applications.length === 1) {
      ...
      if (type === 'object' || type === 'string' || type === 'integer' || type === 'boolean' || type === 'file' || type === 'enum') {
        return {
...
function parseTypedef (tags) {
  const tagType = tags.find(t => t.title === 'typedef')
  const typeName = tagType['name']
  let details = {
    required: [],
    properties: {}
  }
  if (tagType.type && tagType.type.name) {
    details.allOf = [{ $ref: '#/definitions/' + tags[0].type.name }]
  }
  for (let i = 1; i < tags.length; i++) {
    if (tags[i].title === 'description') {
      details.description = tags[i].description
    }
    if (tags[i].title === 'property') {
      ...
      if (schema) {
        if ('description' in tags[i]) {
          schema.description = tags[i].description
        }
        details.properties[propName] = schema
      } else {
        const type = parseType(tags[i].type)
        const parsedDescription = (tags[i].description || '').split(/-\s*eg:\s*/i)
...
function parseEnums (description) {
  let enums = ('' + description).split(/-\s*eg:\s*/i)
  if (enums.length < 2) {
    return []
  }
...
function fileFormat (comments) {
  ...
  for (let i in comments) {
    ...
    if (i === 'tags') {
      if (!Array.isArray(comments[i]) || !comments[i].find(t => t.title === 'swagger')) continue
      if (comments[i].length > 0 && comments[i].find(t => t.title === 'typedef')) {
      ...
      for (let j in comments[i]) {
        ...
        if (title === 'param') {
          ...
          if (!schema) {
            ...
            if (properties.type === 'enum' || ('items' in properties && properties.items.type === 'enum')) {
              ...
              if (properties.type === 'enum') {
                ...
                properties.items.type = parsedEnum.type
                properties.items.enum = parsedEnum.enums
              }
            }
          } else {
            if (schema.type === 'enum' || ('items' in schema && schema.items.type === 'enum')) {
              let parsedEnum = parseEnums(comments[i][j]['description'])
              if (schema.type === 'enum') {
                schema.type = parsedEnum.type
                schema.enum = parsedEnum.enums
              } else {
                schema.items.type = parsedEnum.type
                schema.items.enum = parsedEnum.enums
              }
            }
            if ('$ref' in schema || ('items' in schema && '$ref' in schema.items)) {
        ...
        if ((title === 'desc' || title === 'description') && route) {
          parameters[route.uri][route.method]['description'] = comments[i][j]['description']
        }
 ...
 * @param {string} format 
 */
function patchParameterCollectionFormat(paths, format) {
  if (!format) return
  switch (format) {
    case 'csv':
    case 'tsv':
  ...
  for (const uriPath in paths) {
    for (const method in paths[uriPath]) {
      if ('parameters' in paths[uriPath][method] && Array.isArray(paths[uriPath][method].parameters)) {
        paths[uriPath][method].parameters.forEach(function (param) {
          if (param.in === 'query' && param.type === 'array') {
...
/**
 * @typedef SwaggerDefinitionInfo
 * @property {string} title
 * @property {string} version
 * @property {string=} description 
 */
/**
 * @typedef SwaggerSecurityDefinition
 ...
 * @property {SwaggerDefinitionInfo} info 
 */

/**
 * @typedef GenerateSpecOptions
 * @property {SwaggerDefinition} swaggerDefinition
 * @property {{[key: string]: SwaggerSecurityDefinition}} securityDefinitions
 * @property {string} basedir
 * @property {string[]} files
 * @property {string=} parametersArrayStyle - eg: csv,tsv,ssv,pipes,multi
 */

/**
 * Generates the swagger spec
 * @function
 * @param {GenerateSpecOptions} options - Configuration options
...
async function generateSpec (options) {
  ...
  swaggerObject = await new Promise((resolve, reject) => {
    parser.parse(swaggerObject, function (err, api) {
      if (err) reject(err)
      patchParameterCollectionFormat(api.paths, options.parametersArrayStyle)
      resolve(api)
    })
  })
